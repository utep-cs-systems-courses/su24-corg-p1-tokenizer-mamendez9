...
char *copy_str (char *inStr, short len)
{
  ...
  /*Iterates over the length of the string updating the malloc values */
  for (int i = 0; i < len; i++)
    {
      *strp++ = *inStr++;
    }
...
{
  /* We point a double pointer to the allocated memory of char pointers */
  int num_tokens = count_tokens(str);
  char **final_token = malloc((num_tokens + 1) * sizeof(char *));
  char **token_ptr = final_token;
